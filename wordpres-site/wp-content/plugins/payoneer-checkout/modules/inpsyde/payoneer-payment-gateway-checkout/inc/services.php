...
CSS
), 'checkout.gateway_icon_elements_filenames' => new Factory(['checkout.amex_icon_enabled', 'checkout.jcb_icon_enabled', 'checkout.gateway_icon_elements_filenames_all'], static function (bool $amexEnabled, bool $jcbEnabled, array $icons): array {
        if (!$amexEnabled) {
    ...
    }), 'checkout.gateway_icon_elements_filenames_all' => new Value(['visa.svg', 'mastercard.svg', 'amex.svg', 'jcb.svg']), 'checkout.gateway_icon_elements_all' => new Factory(['checkout.gateway_icon_elements.base_path', 'checkout.gateway_icon_elements_filenames_all'], static function (string $basePath, array $imgFiles): array {
        return array_map(static function (string $file) use ($basePath): string {
            return plugins_url('img/' . $file, $basePath);
        }, $imgFiles);
    ...
    }), 'checkout.payment_field_renderers' => new Value([]), 'checkout.http_request_timeout' => new Value(70), 'checkout.notification_received' => new Factory(['checkout.notification_received.option_name'], static function (string $optionName): bool {
