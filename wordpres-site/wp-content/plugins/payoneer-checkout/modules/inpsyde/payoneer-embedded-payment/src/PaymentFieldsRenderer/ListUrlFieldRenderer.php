...
class ListUrlFieldRenderer implements PaymentFieldsRendererInterface
{
    ...
    public function renderFields(): string
    {
        ...
        $listIdContainer = sprintf('<input type="hidden" name="%1$s" class="%1$s" value="%2$s" %3$s="%5$s" %4$s="%6$s">', esc_attr($this->listUrlContainerId), esc_url_raw($listSessionUrl), $this->listUrlContainerIdAttributeName, $this->listUrlContainerEnvAttributeName, esc_attr($listSessionLongId), esc_attr($listSessionEnvironment));
